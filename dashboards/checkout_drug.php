...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['drug']) && isset($_POST['quantity']) && $_POST['csrf_token'] === $_SESSION['csrf_token']) {
    ...
    if ($row && $row['current_stock'] >= $quantity) {
        ...
        // Log checkout with direct query
        $checkout_time = date('Y-m-d H:i:s');
        $log_query = "INSERT INTO drug_checkouts (drug_name, quantity, name, department, checkout_time) VALUES ('$drug_name', $quantity, '$name', '$department', '$checkout_time')";
        $log_result = mysqli_query($conn, $log_query);
        $log_success = ($log_result !== false);
        if ($log_success) {
            $last_id = mysqli_insert_id($conn);
            error_log("Checkout logged: id=$last_id, drug=$drug_name, quantity=$quantity, name='$name', department=$department, time=$checkout_time");
        } else {
            error_log("Failed to log checkout: " . mysqli_error($conn));
        }
        ...
        if ($update_success && $log_success) {
            ...
            echo json_encode(['error' => 'Failed to process checkout: ' . ($update_success ? '' : 'Stock update failed') . ($log_success ? '' : ' Log insert failed')]);
        }
    } else {
