...
// Admin can see all requests, other departments see only their own
...
<div class="border rounded p-4 shadow-sm">
    <div class="flex justify-between items-center">
        <div>
            <p class="font-semibold text-gray-800"><?php echo htmlspecialchars(cleanDrugName($row['drug_name'])); ?>
                (<?php echo $row['quantity']; ?> units)</p>
            <p class="text-sm text-gray-600">From: <?php echo htmlspecialchars($row['from_department']); ?> → To:
                <?php echo htmlspecialchars($row['to_department']); ?></p>
        ...
        <div class="flex items-center gap-2">
            ...
            <?php if ($row['status'] === 'Pending' && ($row['from_department'] === $user_department || $user_department === 'Admin')) { ?>
            <button onclick="approveRequest(<?php echo intval($row['id']); ?>)"
                class="bg-green-600 text-white px-3 py-1 text-sm rounded hover:bg-green-700">Approve</button>
            <button onclick="rejectRequest(<?php echo intval($row['id']); ?>)"
                class="bg-red-600 text-white px-3 py-1 text-sm rounded hover:bg-red-700">Reject</button>
            <?php } ?>
            <?php if ($row['status'] === 'Pending' && $row['to_department'] === $user_department) { ?>
            <button onclick="cancelRequest(<?php echo intval($row['id']); ?>)"
                class="bg-gray-600 text-white px-3 py-1 text-sm rounded hover:bg-gray-700">Cancel</button>
            <?php } ?>
        </div>
    </div>
</div>
