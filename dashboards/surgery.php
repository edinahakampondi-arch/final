...
error_log("[$user_department] Total: $total_drugs | Critical: $critical_stock | Safe: $safe_stock");
...
<body class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
    <div class="container mx-auto p-6">
        ...
        <section class="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
            <div class="bg-white shadow-lg hover:shadow-xl transition-shadow rounded-lg">
                ...
                <div class="p-4 pt-0">
                    <div class="text-2xl font-bold text-blue-600" id="total-drugs-count">
                        <?php echo htmlspecialchars($total_drugs); ?></div>
                    <p class="text-xs text-gray-500">In Surgery</p>
                </div>
            </div>
            <div class="bg-white shadow-lg hover:shadow-xl transition-shadow rounded-lg">
                <div class="flex flex-row items-center justify-between p-4">
                ...
                <div class="p-4 pt-0">
                    <div class="text-2xl font-bold text-red-600" id="critical-stock-count">
                        <?php echo htmlspecialchars($critical_stock); ?></div>
    ...
    ws.onmessage = (event) => {
        ...
        if (data.type === 'communications') {
            ...
            $('#total-drugs-count').text(data.stats.total_drugs);
            $('#critical-stock-count').text(data.stats.critical_stock);
            $('#safe-stock-count').text(data.stats.safe_stock || 0);
        }
    };
    ws.onclose = () => {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data && Array.isArray(data) && data.length > 0) {
                    ...
                    container.html('<p class="text-gray-600">No drugs found for this department.</p>');
                }
            },
            error: function(xhr, status, error) {
                console.error('Search error:', status, error, xhr.responseText);
                $('#inventory-cards').html(
                    '<p class="text-gray-600">No drugs found for this department.</p>');
            }
        });
    }
