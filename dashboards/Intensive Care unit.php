        ...
        <section class="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
            <div class="bg-white shadow-lg hover:shadow-xl transition-shadow rounded-lg">
                ...
                <div class="p-4 pt-0">
                    ...
                    <p class="text-xs text-gray-500">In ICU</p>
                </div>
            </div>
            <div class="bg-white shadow-lg hover:shadow-xl transition-shadow rounded-lg">
                <div class="flex flex-row items-center justify-between p-4">
    ...
    function updateAlerts(alerts) {
        ...
        if (alerts && Array.isArray(alerts)) {
            alerts.forEach(alert => {
                ...
                const cleanAlertName = alert.drug_name ? alert.drug_name.replace(/Suppository/gi, '').trim() :
                    '';
                container.append(`
                        <div class="border-l-4 ${statusColor} p-4 rounded-r-lg">
                            <div class="flex justify-between items-center">
                                <span><strong>${cleanAlertName}</strong> in ${alert.department} - Stock: ${alert.current_stock} units</span>
                ...
                success: function(response) {
                    ...
                    response.forecasts.forEach((forecast, index) => {
                        ...
                        if (method.includes('autoets')) {
                            ...
                            'projection') || method.includes('linear') || method.includes(
                                'growth')) {
